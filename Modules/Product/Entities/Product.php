...
class Product extends Model
{
     ...
     */
    protected $appends = [
        'base_image', 'base_image_2','formatted_price', 'rating_percent', 'is_in_stock', 'is_out_of_stock',
    ...
    public function scopeWithBaseImage($query)
    {
        $query->with(['files' => function ($q) {
            $q->wherePivotIn('zone', ['base_image', 'base_image_2']);
        }]);
    }
    ...
    public function getPricePercentConvertAttribute()
    {
        ...
        if($this->hasSpecialPrice() && $this->special_price_type === 'fixed'){
            return  round(100 - ($this->special_price->amount() / $this->price->amount() * 100 ));
        }
    }
}
