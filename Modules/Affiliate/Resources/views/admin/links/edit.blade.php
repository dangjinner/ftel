...
@section('content')
    <form method="POST" action="{{ route('admin.affiliate_links.update', ['id' => $affiliateLink->id]) }}" class="form-horizontal"
          id="affiliate-link-edit-form" novalidate>
        {{ csrf_field() }}
        {{ method_field('put') }}
...
@push('scripts')
    <script>
        ...
        $('#affiliate-link-edit-form').submit(function(e) {
            e.preventDefault();
