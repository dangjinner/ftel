...
<div class="button-hidden-sidebarright" style="text-align: left;font-size: 1.5rem;margin-left: -80px;">
...
<script>
    ...
    function callAjax(name, post_id, flag)
    {
        $.ajax({
    ...
    $("#edit-slug-box .cancel").on("click", function () {
        ...
        var a = $("#sample-permalink");
        a.html('<a class="permalink" href="' + $("#slug_id").data("view") + e.replace("/", "") + '">' + a.html() + "</a>"),
    ...
    $("#name").blur(function () {
        if ($("#edit-slug-box").hasClass("hidden")) {
            var name = $("#name").val();
            var post_id = $("#slug_id").data("id");
            if(null !== name && "" !== name){
                callAjax(name, post_id, true);
            }
        }
    });
