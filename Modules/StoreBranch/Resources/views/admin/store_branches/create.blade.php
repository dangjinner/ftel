...
@component('admin::components.page.header')
    @slot('title', trans('admin::resource.create', ['resource' => trans('storebranch::store_branches.store_branch')]))

    <li><a href="{{ route('admin.store_branches.index') }}">{{ trans('storebranch::store_branches.store_branches') }}</a></li>
    <li class="active">{{ trans('admin::resource.create', ['resource' => trans('storebranch::store_branches.store_branch')]) }}</li>
@endcomponent

@section('content')
    <form method="POST" action="{{ route('admin.store_branches.store') }}" class="form-horizontal" id="store-branch-create-form" novalidate>
        {{ csrf_field() }}
