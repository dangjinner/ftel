...
<link rel="stylesheet" href="{{ Theme::url('assets/css/slug.css') }}">
<style>
    .form-group.status {
...
@endpush
@section('content')
<form method="POST" action="{{ route('admin.pages.update', $page) }}" class="form-horizontal" id="page-edit-form"
    novalidate>
    {{ csrf_field() }}
    {{ method_field('put') }}
    ...
    <div class="row wrapper-content">
        <div class="col-lg-9 col-md-9 sidebar-left">
            @include('page::admin.pages.tabs.general')
            @include('page::admin.pages.tabs.shortcode_custom')
            @include('page::admin.pages.tabs.customize')
            @include('page::admin.pages.tabs.seo')
                            ...
                            <div class="panel-body">
                                ...
                                <div class="form-group">
                                    <label for="meta-title" class="col-md-5 control-label text-left">
                                        {{ trans('page::sidebar.is_thumbnail') }}
                                    </label>
                                ...
                                <div class="form-group">
                                    <label for="meta-title" class="col-md-12 control-label text-left">
                                        {{ trans('page::sidebar.sidebar_layout') }}
                                    </label>
        ...
        <div class="col-lg-3 col-md-3 sidebar-right">
            <div class="form-group status">
                <div class="control-label text-left">{{trans('page::attributes.is_active')}}</div>
                <div class="checkbox">
                    <input type="hidden" value="0" name="is_active">
                    <input type="checkbox" name="is_active" class="" id="is_active" value="1">
                    <label for="is_active">{{ trans('page::pages.form.enable_the_post') }}</label>
                </div>
            </div>

            <div class="form-group categories">
                <div class="control-label text-left">{{trans('page::attributes.groups')}}</div>
                <div class="button-expand" style="margin-bottom: 20px;">
                    <a class="btn-collapse" href="#">Collapse</a>
                    <a class="btn-expand" href="#" style="margin-left:15px;">Expand</a>
                </div>
                @foreach ($groups as $key => $value)
                <div class="form-check {{ strpos($value, '––') > 0 ? 'is-child' : '' }}">
                        ...
                        <div id="box-puslish" class="panel-collapse collapse in" aria-expanded="true" style="">
                            <div class="panel-body" style="padding-left: 5px; padding-right: 5px;">
                                <div class="column-first-button">
                                    ...
                                    <button type="button" class="btn btn-light" id="page-preview">Preview</button>
                                </div>
                                    ...
                                    <div class="collapse" id="edit_timestamp">
                                            ...
                                            <div class="timestamp-wrap">
                                                <label>
                                                    <select id="mm" name="mm"
                                                        data-month="{!! date('m', strtotime($page->created_at)) !!}">
                                                ...
                                                <label>
                                                    <input type="text" id="dd"
                                                        value="{{ date('d', strtotime($page->created_at)) }}" size="2"
                                                        maxlength="2">
                                                </label>,
                                                <label>
                                                    <input type="text" id="yy"
                                                        value="{{ date('Y', strtotime($page->created_at)) }}" size="4"
                                                        maxlength="4">
                                                </label>at
                                                <label>
                                                    <input type="text" id="hh"
                                                        value="{{ date('h', strtotime($page->created_at)) }}" size="2"
                                                        maxlength="2">
                                                </label>:
                                                <label>
                                                    <input type="text" id="mn"
                                                        value="{{ date('i', strtotime($page->created_at)) }}" size="2"
                                                        maxlength="2">
                                                </label>
                                            </div>
                                        </div>
                                        <p>
                                            <a data-toggle="collapse" href="#edit_timestamp" role="button"
                                                aria-expanded="false" aria-controls="edit_timestamp"
                                                class="btn btn-default" id="timestamp-ok">OK</a>
                                            <a data-toggle="collapse" href="#edit_timestamp" role="button"
            ...
            {{-- @include('page::admin.pages.tabs.images')
            <div class="form-group">
                <div class="content-accordion panel-group options-group-wrapper" id="option-0">
                    <div class="panel panel-default option">
                        <div class="panel-heading">
                            <h4 class="panel-title">
                                <a data-toggle="collapse" href="#box-video" aria-expanded="true" class="" style="position: relative;
                                                                        text-decoration: none;
                                                                        overflow: hidden;">
                        ...
                        <div id="box-video" class="panel-collapse collapse" aria-expanded="true" style="">
                            <div class="panel-body" style="padding-left: 5px; padding-right: 5px;">
                                <div class="form-group">
                                    <label for="video" class="col-md-12 control-label text-left">
                                        URL
                                    </label>
                                    <div class="col-md-12">
...
<script>
    $(document).ready(function(){
            $('.btn-collapse').click(function(e){
                e.preventDefault();
                $('.form-check.is-child').fadeOut();
            });
            $('.btn-expand').click(function(e){
                e.preventDefault();
                $('.form-check.is-child').fadeIn();
            });
        });
        function callAjax(name, page_id, flag)
        {
            $.ajax({
                url: $("#page_slug_id").data("url"),
                type: "POST",
                data: { name },
                success: function (response) {
                    var a = $("#sample-page-permalink");
                    var t = $("#page_slug_id");
                    flag ? a.find(".permalink").prop("href", t.data("view") + response.replace("/", "")) : a.html('<a class="permalink" target="_blank" href="' + t.data("view") + response.replace("/", "") + '">' + a.html() + "</a>");
                        $(".page-url-seo p").text(t.data("view") + response.replace("/", ""));
                        $("#editable-page-name").text(response);
                        $("#current-page-slug").val(response);
        ...
        $("#change_page_slug").on("click", function (e) {
            $(".default-slug").unwrap();
            var a = $("#editable-page-name");
            a.html('<input type="text" id="new-post-slug" class="form-control" value="' + a.text() + '" autocomplete="off">');
            $("#edit-slug-box .cancel").css('display', 'inline-block');
            $("#edit-slug-box .save").css('display', 'inline-block');
            $(e.currentTarget).hide();
        });
    
        $("#edit-slug-box .cancel").on("click", function () {
            var e = $("#current-page-slug").val();
            var a = $("#sample-page-permalink");
            a.html('<a class="permalink" href="' + $("#page_slug_id").data("view") + e.replace("/", "") + '">' + a.html() + "</a>"),
                $("#editable-page-name").text(e);
                $("#edit-slug-box .cancel").hide();
                $("#edit-slug-box .save").hide();
                $("#change_slug").show();
        });
        $("#name").blur(function () {
            if ($("#edit-slug-box").hasClass("hidden")) {
                var name = $("#name").val();
                if(null !== name && "" !== name){
                    callAjax(name, 0, true);
                }
            }
        });
    
        $("#edit-slug-box .save").on("click", function () {
            var a = $("#new-post-slug").val();
            callAjax(a, 0, false);
        });
</script>
<script>
...
<script>
    function selectDate(){
        ...
        $(document).ready(function(){
            $('#page-preview, .permalink').click(function(e){
                e.preventDefault();
                ...
                var data = tinyMCE.get("body").getContent({format: 'html'});
                $.ajax({
                    method:'POST',
                    data: {
                        data,
