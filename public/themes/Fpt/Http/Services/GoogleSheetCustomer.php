...
class GoogleSheetCustomer
{
    ...
    {
        $this->spreadSheetId = config('services.google_sheet_customer.id');
    ...
    public function readGoogleSheet()
    {
        ...
        $range = 'dangkyonline!A1:' . $dimensions['colCount'];
    ...
    private function getDimensions($spreadSheetId)
    {
        $rowDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'dangkyonline!A:A', 'majorDimension' => 'COLUMNS']
        );
        ...
        $colDimensions = $this->googleSheetService->spreadsheets_values->batchGet(
            $spreadSheetId,
            ['ranges' => 'dangkyonline!1:1', 'majorDimension' => 'ROWS']
        );
