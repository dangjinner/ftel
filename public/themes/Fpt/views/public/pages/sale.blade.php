        ...
        <div class="row">
            ...
            <div class="col-lg-4 styled-select">
                <select name="area" class="select-province">
                    ...
                    @endphp
                    <option value="{{ url()->current().'/'.$saleGroups->first()->slug.'/'. 'fpt-'.$areaSlug}}">{{ $value }}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
<section class="news news--1">
    <div class="container">
        <div class="row mt-20 mb-20">
            ...
            $patternEndBox = '/\[\/box\]/';
            @endphp
            @if (count($post_sale) > 0)
                @foreach ($post_sale as $item)
                    @if ($loop->first)
                        ...
                        <div class="col-lg-4 news__first col-md-6 col-sm-6">
                            ...
                            <div class="single-blog">
                                <div class="blog-content">
                                    ...
                                    <div><a href="{{ route('pages.news.show', ['slug' => $item->slug]) }}" class="news__more"> Tìm hiểu</a></div>
                                </div>
                            </div>
                            <!-- Single Blog End -->
                        </div>
