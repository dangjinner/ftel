        ...
        <div class="pricing--4 slick-custom slick-custom-default">
            ...
            <div class="pricing__col {{ getColorClass($i) }}">
                <div class="pricing__inner">
                    ...
                    @php
                    $price = number_format($service->price->amount(), 0,",",".");
                    if($area_id != null){
                        if($service->areas($area_id)->orderBy('price_area_discount','ASC')->first()->pivot->price_area_discount){
                            $price = number_format($service->areas($area_id)->orderBy('price_area_discount','ASC')->first()->pivot->price_area_discount,0,",",".");
                        }else{
                            $price = number_format($service->areas($area_id)->orderBy('price_area_discount','ASC')->first()->pivot->price_area,0,",",".");
                        }
                    }else{
                        $price = number_format($service->price->amount(),0,",",".");
                    }
                    ...
                    @else
                        <h4>{{ number_format($service->price->amount(),0,",",".") }}</h4>
                    @endif --}}
                    {!! sprintf($service->feature, ($price . ' đ')) !!}
                    {{-- {!! $service->feature !!} --}}
                    <div class="bottom">
