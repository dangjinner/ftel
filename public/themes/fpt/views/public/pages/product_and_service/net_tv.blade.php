...
<div class="single-banner">
    <img src="{{ $category_services->banner->path ?? $category_services->parent->banner->path }}" alt="Đăng ký online FPT" class="img-fluid">
    <div class="bg-menu-banner">
        <div class="container">
            <div class="row">
                @foreach($primaryMenu[0]->children()->get() as $menuItem)
                <div class="col-lg-3 col-md-3 col-sm-3 col-6">
                    <a href="{{ $menuItem->url }}" @if($loop->iteration == 1) class="active" @endif>{{ $menuItem->name
                        }}</a>
                </div>
                @endforeach
            </div>
                            ...
                            <div class="price">
                                @if($area_id != null)
                                    @if ($service->areas($area_id)->count() > 0)
                                        @if ($service->areas($area_id)->orderBy('price_area','ASC')->first()->pivot->price_area_discount)
                                            <h4>{{ number_format($service->areas($area_id)->orderBy('price_area','ASC')->first()->pivot->price_area_discount,0,",",".") }}
                                            </h4>
                                        @else
                                            <h4>{{ number_format($service->areas($area_id)->orderBy('price_area','ASC')->first()->pivot->price_area,0,",",".") }}
                                            </h4>
                                        @endif
                                    @else
        ...
        <div class="block-title block__orange">
            <h2>
                <span class="block__image">
                    <img alt="Giới thiệu" src="{{ v(theme::url('assets/images/icon/function fpt.png'))}}">
                </span>
                TÍNH NĂNG
            </h2>
        </div>
